...
public class slotActivity extends ActionBarActivity {
...
//            {
					...
					if (Integer.parseInt(user.getProperty("balance").toString())>0)
						...
						Backendless.UserService.update( user, new AsyncCallback<BackendlessUser>()
					{
						...
						Toast.makeText(getApplicationContext(),"You have no credit left, You Need to rebuy", Toast.LENGTH_SHORT).show();
	...
	// Backendless.UserService.login( "sarah@backendless.com", "my_super_password", new AsyncCallback<BackendlessUser>()
					...
					mResultsText.setText(slotActivity.this.getResources().getString(R.string.try_again_text));
			BackendlessUser user=Backendless.UserService.CurrentUser(); // I am always get null
	...
	public SpannableString getActionBarSpannableTitle(String title) {
		...
		span.setSpan(new com.sourcey.materiallogindemo.TypefaceSpan(this, ROBOTO_BOLD ), 0, title.length(),
	...
	private void setSuccessResultMessage(int position) {
		...
		if (position == 1) {
			beverage = res.getString(R.string.beverage_ESPRIT);

		} else if (position == 2) {
			beverage = res.getString(R.string.beverage_seven);

		} else if (position == 3) {
			beverage = res.getString(R.string.beverage_cherries);
		} else {
			mResultsText.setVisibility(View.INVISIBLE);
			return;
		}
